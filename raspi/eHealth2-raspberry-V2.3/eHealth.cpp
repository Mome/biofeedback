...
//***************************************************************
	...
	void eHealthClass::readPulsioximeter(void)
	{
		...
		for (int i = 0; i<99 ; i++) { // read all the led's of the module
			A = !digitalRead(13);
			B = !digitalRead(12);
			C = !digitalRead(11);
			D = !digitalRead(10);
			...
			delayMicroseconds(86); //300 microseconds			
			
		}

			SPO2 = 10 * digito[93] + digito[32];
			BPM  = 100 * digito[26] + 10 * digito[8] + digito[2];
